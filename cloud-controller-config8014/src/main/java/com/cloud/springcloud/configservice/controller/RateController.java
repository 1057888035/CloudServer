...
public class RateController {
    ...
    @GetMapping(value = "/getRateByNamessss/{name}",name = "根据名称获取费率")
    public BigDecimal getRateByNamessss(@PathVariable("name") String name){
        QueryWrapper<Rate> wrapper = new QueryWrapper();
        wrapper.eq("R_NAME",name);
        Rate one = rateService.getOne(wrapper);
        return one.getRRate();
    ...
    @PostMapping(value = "/update",name = "修改费率")
    public CommonResult getRateByName(@RequestBody Rate rate){
